...
pub async fn list_interfaces() {
    ...
    while let Some(dart_signal) = receiver.recv().await {
        ...
        let mut arp_attack: Process = vec![&req.exec_path, "-l"].into();
        let outputs;
        match arp_attack.spawn_and_stream() {
            Ok(u) => {
                outputs = u
...
pub async fn do_attack() {
    ...
    while let Some(dart_signal) = receiver.recv().await {
        let req = dart_signal.message;
        arp_attack = vec![&req.exec_path, "-l"].into();
        arp_attack.abort();
        let outputs = arp_attack.spawn_and_stream().unwrap().for_each(|e| {
            InterfaceRes { interfaces: vec![] }.send_signal_to_dart(None);
            future::ready(())
        });
        tokio::spawn(outputs);
    }
